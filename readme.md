...
If you're using a JavaScript templating library like Handlebars.js, the templating expression's opening braces must to be escaped with backslashes, like `\{\{` or `\{\%`. They'll be output as normal `{{ }}` expressions in HTML:

```
`{% image {{ post.featured_image }} alt="\{\{ user.name }}" %}`.
```
